...
// TestRijndaelTestSet1 implements Rijndael test data in
...
func TestRijndaelTestSet1(t *testing.T) {
	...
	fmt.Printf("Rijndael (Actual)  : %x\n", res)
	fmt.Printf("Rijndael (Expected): %x\n", ciphertext)
...
func TestF1TestSet1(t *testing.T) {
	...
	opc := strToHexArray("cd63cb71954a9f4e48a5994e37a02baf")
	...
	e1 := strToHexArray("9e2980c59739da67b136355e3cede6a2")
	fmt.Printf("1st encryption (expected)  : %x\n", e1)
	...
	if !bytes.Equal(macAE, macAA) {
		t.Error("3GPP TS 35.207 Milenage f1/f1* Test Set 1 failed\n")
	} else {
		fmt.Print("3GPP TS 35.207 Milenage f1/f1* Test Set 1 Passed\n")
	}
}
